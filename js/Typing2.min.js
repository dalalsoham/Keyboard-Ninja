        ...
        _totalWords.textContent = d,
            ...
            this.typing || (this.typing = !0,
                ...
                input.addEventListener('keypress', f => {
                    f.preventDefault(), f = f || window.event; const g = f.which || f.keyCode, h = String.fromCharCode(g);
                    if (h === this.quote.charAt(this.index)) {
                        ' ' === h && 32 === g && (this.words++, _writtenWords.textContent = this.words), this.index++; const j = this.quote.substring(this.index, this.index + this.quote.length);
                        if (input.textContent = j, output.innerHTML += h, this.correctIndex++, this.index === this.quote.length) return this.stop(), void this.finish();
                        sound && (keyClick.currentTime = 0, keyClick.play())
                    }
                    else output.innerHTML += `<span class="text-danger">${h}</span>`, this.errorIndex++, _errors.textContent = this.errorIndex, this.correctIndex--, sound && (keyBeep.currentTime = 0, keyBeep.play());
                    this.cpm = 5 < this.correctIndex ? Math.floor(60 * (this.correctIndex / this.duration)) : 0, _cpm.textContent = this.cpm, this.wpm = Math.round(this.cpm / 5), _wpm.textContent = this.wpm, this.accuracyIndex = 5 < this.correctIndex ? Math.round(100 * this.correctIndex / this.index) : 0, 0 < this.accuracyIndex && Number.isInteger(this.accuracyIndex) && (_accuracy.innerHTML = `${this.accuracyIndex}<span class="small">%</span>`)
                }))
    } stop() {
        return clearInterval(this.interval), this.interval = null, this.typing = !1, _timer.textContent = '0', btnPaly.remove(), input.remove(), btnRefresh.classList.add('active'), inputFull.classList.remove('d-none'), void (inputFull.innerHTML = `&#8220;${this.quote}&#8221; 
    <span class="d-block small text-muted text-right pr-3">&ndash; ${this.author}</span></div> `)
    } finish() {
...
const savedWPM = localStorage.getItem('WPM') || 0; select('#result').innerHTML = `<li>${savedWPM}</li>`;